...
export const Radar = ({ className }: any) => {
  const circles = new Array(9).fill(1);
  return (
    <div
      className={twMerge(
      ...
      <div
        ...
        <div className="relative z-40 h-[1px] w-full bg-gradient-to-r from-muted/10  via-indigo-500/35 to-muted/10" />
      </div>
      {/* concentric circles */}
      {circles.map((circle, idx) => (
        <Circle
          style={{
            height: `${(idx + 1) * 5}rem`,
            width: `${(idx + 1) * 5}rem`,
            border: `1px solid rgba(59, 130, 246, 0.01})`,
            opacity: 0.5,
          }}
          key={`motion-${idx}`}
          idx={idx}
